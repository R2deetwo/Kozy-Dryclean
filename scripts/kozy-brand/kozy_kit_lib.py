#!/usr/bin/env python3
"""kozy_kit_lib.py — shared components for the Kozy Care brand kit v6 build.

Extracted from the existing v5 kit HTML so every new piece stays pixel-consistent
with the collateral the client already approved.
"""
import base64
import re
...
CREAM = '#F5F1E8'
WHITE = '#FFFFFF'
# text tints used across the existing kit
TINT_BODY = '#C9D5E6'   # body text on navy
TINT_HI = '#E7EDF5'     # primary text on navy
TINT_HEAD = '#F2F6FB'   # headings on navy
TINT_MUTE = '#9FB0C6'   # muted text on navy
DOTS = '#B8AE9A'        # dotted price leaders on cream

PX_MM = 96 / 25.4  # 3.7795 px per mm (matches every existing piece)

# ------------------------------------------------------------------- K mark SVG
# Kozy Care v4 monogram — stylized K with tapered hanger-wire and ball terminal
# (extracted verbatim from the v5 collateral; fill colour injected per piece).
K_MARK_PATHS = (
...
# hanger-wire sweep (second path of the monogram, verbatim)
K_WIRE_PATH = (
...
MARK_VIEWBOX = '-24 -838 861 896'   # 861x896 units, origin bottom-left (y-up)


def k_mark(fill: str = GOLD, height_px: float = 52.0, style: str = '') -> str:
    """Inline K-mark SVG sized by height, exactly as used in the v5 collateral."""
    return (
        f'<svg viewBox="{MARK_VIEWBOX}" style="height:{height_px}px;display:block;{style}" '
...
# ------------------------------------------------------------------------ QR
_QR_SRC = Path('/home/z/my-project/download/kozy-brand/business-cards/html-source/card-navy-back.html')


def qr_data_uri() -> str:
    """The approved 'scan to book' QR (https://kozycare.ng) as a data URI."""
    src = _QR_SRC.read_text(encoding='utf-8')
    m = re.search(r'src="(data:image/png;base64,[^"]+)"', src)
    if not m:
        raise RuntimeError('QR data URI not found in source card')
    return m.group(1)
...
# ------------------------------------------------------------------ crop marks
def crop_marks(poster_w: float, poster_h: float, trim_w_mm: float, trim_h_mm: float,
               label: str) -> str:
    """Trim-corner crop marks + spec label, identical construction to the v5 kit.

    Geometry: poster includes 5mm marks margin around trim+bleed on every side.
    Trim origin is therefore at 8mm (5mm margin + 3mm bleed) from the poster edge.
    """
    m = PX_MM
    x0 = 8.0 * m                      # trim left
    y0 = 8.0 * m                      # trim top
    x1 = x0 + trim_w_mm * m           # trim right
    y1 = y0 + trim_h_mm * m           # trim bottom
    # marks: 2mm-long ticks, 2mm..4mm from the poster edge (v5 convention)
    def hline(xa, xb, y): return f'<line x1="{xa:.2f}" y1="{y:.2f}" x2="{xb:.2f}" y2="{y:.2f}"/>'
    def vline(x, ya, yb): return f'<line x1="{x:.2f}" y1="{ya:.2f}" x2="{x:.2f}" y2="{yb:.2f}"/>'
    lines = []
    # top / bottom horizontal ticks (aligned with trim top/bottom edges)
    lines += [hline(15.12, 22.68, y0), hline(poster_w - 22.68, poster_w - 15.12, y0)]
    lines += [hline(15.12, 22.68, y1), hline(poster_w - 22.68, poster_w - 15.12, y1)]
    # left / right vertical ticks
    lines += [vline(x0, 15.12, 22.68), vline(x0, poster_h - 22.68, poster_h - 15.12)]
    lines += [vline(x1, 15.12, 22.68), vline(x1, poster_h - 22.68, poster_h - 15.12)]
    label_y = poster_h - (5.0 * m) + 3.2
    return (
        f'<svg class="marks" width="{poster_w}" height="{poster_h}" '
        f'viewBox="0 0 {poster_w} {poster_h}">'
        f'<g stroke="#111827" stroke-width="0.35" opacity="0.9">{"".join(lines)}</g>'
        f'<text x="{poster_w / 2:.2f}" y="{label_y:.2f}" text-anchor="middle" '
        f'font-family="Outfit, Arial, sans-serif" font-size="8.5" fill="#6B7280" '
        f'letter-spacing="1.5">{label}</text></svg>'
    )
...
COMPANY = {
    ...
    'desc': 'PREMIUM DRYCLEANING & LAUNDRY',
